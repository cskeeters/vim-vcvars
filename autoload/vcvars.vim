...
function! vcvars#VcVars()
    ...
    let gitroot = vcvars#Strip(system("cd ".shellescape(l:bufdir)."; git rev-parse --show-toplevel"))
    let isGit = v:shell_error == 0
    let hgroot = vcvars#Strip(system("cd ".shellescape(l:bufdir)."; hg root"))
    ...
    if l:isHg
        let hgbranch = vcvars#Strip(system("cd ".shellescape(l:bufdir)."; hg branch"))
        if v:shell_error != 0
            let hgbranch = ''
        endif
    ...
    if l:isGit
        let gitbranch = vcvars#Strip(system("cd ".shellescape(l:bufdir)."; git rev-parse --abbrev-ref HEAD"))
        if v:shell_error != 0
            let gitbranch = ''
        endif
